    ...
    makeMove( gamestate, moves ){
        ...
        class AlphaBetaNode {
            ...
            heuristic () {
                ...
                console.log(playableLocations)
                for (let i of playableLocations) {
                    console.log("iterating")
                    if (this.gamestate.getOwner(i)) {
                        piecesLeft++
                    }
                    console.log(this.gamestate.whoseTurn())
                    console.log(this.gamestate.getOwner(i))
                    console.log(this.gamestate.getLevel(i))
                    if (this.gamestate.whoseTurn() == 1 && this.gamestate.getOwner(i) == 1 && this.gamestate.getLevel(i) == 0) {
                        console.log("turn:",this.gamestate.whoseTurn())
                        console.log("owner:",this.gamestate.getOwner(i))
                        console.log("level:",this.gamestate.getLevel(i))
                        positionValue = positionValue + parseInt(i, 10)
                    }
                    else if (this.gamestate.whoseTurn() == 2 && this.gamestate.getOwner(i) == 2 && this.gamestate.getLevel(i) == 0) {
                        console.log("turn:",this.gamestate.whoseTurn())
                        console.log("owner:",this.gamestate.getOwner(i))
                        console.log("level:",this.gamestate.getLevel(i))
                        positionValue = positionValue + Math.abs(parseInt(i, 10)-33)
                    }
                    /*if (this.gamestate.whoseTurn() == 1 && this.gamestate.getOwner(i) == 1 && this.gamestate.getLevel(i) != 0) {
                        positionValueKings = positionValueKings + Math.floor(Math.abs(parseInt(i, 10)-33)/2)
                    }
                    else if (this.gamestate.whoseTurn() == 2 && this.gamestate.getOwner(i) == 2 && this.gamestate.getLevel(i) != 0) {
                        positionValueKings = positionValueKings + Math.floor(parseInt(i, 10)/2)
                    }*/
                }

                score = score + (3 * positionValue).toString()
